    ...
    namespace units_impl {
        ...
        protected:
            ...
            static constexpr Unit_T FromValue(T value) {
                ...
                return Unit_T(core::dchecked_cast<int64_t>(value));
            }
            template <typename T,
            ...
            static constexpr Unit_T FromValue(T value) {
                if (value == std::numeric_limits<T>::infinity()) {
                    ...
                    return FromValue(core::dchecked_cast<int64_t>(value));
                }
            }
            ...
            static constexpr Unit_T FromFraction(int64_t denominator, T value) {
                ...
                return Unit_T(core::dchecked_cast<int64_t>(value * denominator));
            }
            template <typename T,
                      typename std::enable_if<std::is_floating_point<T>::value>::type* =
                      nullptr>
            ...
            ToValue() const {
                assert(IsFinite());
                return core::dchecked_cast<T>(value_);
            }
            template <typename T>
            ...
            ToFraction() const {
                assert(IsFinite());
                if (Unit_T::one_sided) {
                    return core::dchecked_cast<T>(
                        DivRoundPositiveToNearest(value_, Denominator));
                } else {
                    return core::dchecked_cast<T>(DivRoundToNearest(value_, Denominator));
                }
            }
